...
<div class="form-field">
   <#if form.mode == "view">
      <div class="viewmode-field">
         ...
         <span id="labelRetailer" class="viewmode-value">${valueToShow?html}</span>
      </div>
   <#else>
...
{
    url: Alfresco.constants.PROXY_URI + "com/alfea/hoteia/poc/standardDataList?type=RETAILER",
    successCallback:
    {
       fn: onRetailerListSuccess,
       scope: this
    },
    failureCallback:
    {
       fn: onRetailerListFailed,
       scope: this
    },
    scope: this,
    noReloadOnAuthFailure: true
 });
function onRetailerListSuccess(p_response){
    ...
    for (var i = 0, len = list.length; i < len; ++i) {
		var param = list[i];
			...
			{
				if("${fieldValue}" == param.id) {
					var mySpan = YAHOO.util.Dom.get("labelRetailer");
...
function onRetailerListFailed(){
	alert("Load retailer list failed");
}
</script>
