...
<div class="form-field">
   <#if form.mode == "view">
      <div class="viewmode-field">
         ...
         <span id="labelMarket" class="viewmode-value">${valueToShow?html}</span>
      </div>
   <#else>
...
{
    url: Alfresco.constants.PROXY_URI + "com/alfea/hoteia/poc/standardDataList?type=MK",
    successCallback:
    {
       fn: onMarketListSuccess,
       scope: this
    },
    failureCallback:
    {
       fn: onMarketListFailed,
       scope: this
    },
    scope: this,
    noReloadOnAuthFailure: true
 });
function onMarketListSuccess(p_response){
    ...
    for (var i = 0, len = list.length; i < len; ++i) {
		var param = list[i];
			...
			{
				if("${fieldValue}" == param.id) {
					var mySpan = YAHOO.util.Dom.get("labelMarket");
...
function onMarketListFailed(){
	alert("Load market list failed");
}
</script>
