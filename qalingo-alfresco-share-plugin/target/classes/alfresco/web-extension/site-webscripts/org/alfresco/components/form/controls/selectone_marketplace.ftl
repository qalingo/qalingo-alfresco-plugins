...
<div class="form-field">
   <#if form.mode == "view">
      <div class="viewmode-field">
         ...
         <span id="labelMarketPlace" class="viewmode-value">${valueToShow?html}</span>
      </div>
   <#else>
...
{
    url: Alfresco.constants.PROXY_URI + "com/alfea/hoteia/poc/standardDataList?type=MP",
    successCallback:
    {
       fn: onMarketPlaceListSuccess,
       scope: this
    },
    failureCallback:
    {
       fn: onMarketPlaceListFailed,
       scope: this
    },
    scope: this,
    noReloadOnAuthFailure: true
 });
function onMarketPlaceListSuccess(p_response){
    ...
    for (var i = 0, len = list.length; i < len; ++i) {
		var param = list[i];
			...
			{
				if("${fieldValue}" == param.id) {
					var mySpan = YAHOO.util.Dom.get("labelMarketPlace");
...
function onMarketPlaceListFailed(){
	alert("Load market place list failed");
}
</script>
