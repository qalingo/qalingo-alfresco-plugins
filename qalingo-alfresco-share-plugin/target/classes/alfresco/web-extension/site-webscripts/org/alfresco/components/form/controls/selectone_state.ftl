...
<div class="form-field">
   <#if form.mode == "view">
      <div class="viewmode-field">
         ...
         <span id="labelState" class="viewmode-value">${valueToShow?html}</span>
      </div>
   <#else>
...
{
    url: Alfresco.constants.PROXY_URI + "com/alfea/hoteia/poc/standardDataList?type=STATE",
    successCallback:
    {
       fn: onStateListSuccess,
       scope: this
    },
    failureCallback:
    {
       fn: onStateListFailed,
       scope: this
    },
    scope: this,
    noReloadOnAuthFailure: true
 });
function onStateListSuccess(p_response){
    ...
    for (var i = 0, len = list.length; i < len; ++i) {
		var param = list[i];
			...
			{
				if("${fieldValue}" == param.id) {
					var mySpan = YAHOO.util.Dom.get("labelState");
