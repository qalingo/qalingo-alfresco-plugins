...
<div class="form-field">
   <#if form.mode == "view">
      <div class="viewmode-field">
         ...
         <span id="labelMarketArea" class="viewmode-value">${valueToShow?html}</span>
      </div>
   <#else>
...
{
    url: Alfresco.constants.PROXY_URI + "com/alfea/hoteia/poc/standardDataList?type=MA",
    successCallback:
    {
       fn: onMarketAreaListSuccess,
       scope: this
    },
    failureCallback:
    {
       fn: onMarketAreaListFailed,
       scope: this
    },
    scope: this,
    noReloadOnAuthFailure: true
 });
function onMarketAreaListSuccess(p_response){
    ...
    for (var i = 0, len = list.length; i < len; ++i) {
		var param = list[i];
			...
			{
				if("${fieldValue}" == param.id) {
					var mySpan = YAHOO.util.Dom.get("labelMarketArea");
...
function onMarketAreaListFailed(){
	alert("Load market area list failed");
}
</script>
