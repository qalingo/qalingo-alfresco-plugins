...
<div class="form-field">
   <#if form.mode == "view">
      <div class="viewmode-field">
         ...
         <span id="labelCategorie" class="viewmode-value">${valueToShow?html}</span>
      </div>
   <#else>
...
{
    url: Alfresco.constants.PROXY_URI + "com/alfea/hoteia/poc/standardDataList?type=CATEGORIE",
    successCallback:
    {
       fn: onCategorieListSuccess,
       scope: this
    },
    failureCallback:
    {
       fn: onCategorieListFailed,
       scope: this
    },
    scope: this,
    noReloadOnAuthFailure: true
 });
function onCategorieListSuccess(p_response){
    ...
    for (var i = 0, len = list.length; i < len; ++i) {
		var param = list[i];
			...
			{
				if("${fieldValue}" == param.id) {
					var mySpan = YAHOO.util.Dom.get("labelCategorie");
...
function onCategorieListFailed(){
	alert("Load state list failed");
}
</script>
